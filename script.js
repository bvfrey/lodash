...
var tmp = JSON.stringify(people);
var people= JSON.parse(tmp);

$(function(){

    function format_for_print(header, result_array) {
        var my_html = "<h3>" + header + "</h3>";
        my_html += "<ol>";
    ...
    var skills = _.sortBy(_.uniq(_.flatMap(people, 'skills')));
    console.log(skills);
    $('#placeholder_skills').html(format_for_print("List of skills (uniq and sorted)", skills));

    var popularity_rating = _.flatMap(_.orderBy(_.map(people, function(value) {
        return ({name: value.name, popularity: _.size(value.friends)});
    }), 'popularity', 'desc'), 'name')
    console.log(popularity_rating);
    $('#placeholder_names').html(format_for_print("Popularuty Rating", popularity_rating));

    var friends = _.uniq(_.flatMap(_.flatMap(people, 'friends'), 'name'));
    console.log(friends);
    $('#placeholder_friends').html(format_for_print("Friends", friends));
});
